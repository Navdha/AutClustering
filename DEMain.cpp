...
ofstream trackMinMaxAvg;
ofstream trackArchive;
//int changeCounter =0;
...
DEMain::DEMain(int dim, int** placeholder, Item** items, int itemSize, int validityIndex, Parameters param) {
  ...
  p = new Population(param.kmax, dim, 10*dim);
...
void DEMain::setup(double min[], double max[]) {
  ...
  int bestInd = 0;
  int worstInd = 0;
  trackMinMaxAvg.open("popVaryingWine.txt", fstream::app);
  trackMinMaxAvg << "Initial average fitness is " << avgStart/pSize << endl;
  ...
  cout << "Setup best chromosome index " << p->bestChromosomeIndex << endl;
  trackMinMaxAvg << "Max fitness initially " << fitness << " for index " << bestInd << " and # of clusters " << p->chromosome[bestInd]->active_ctr <<  endl;
  ...
  trackMinMaxAvg << "Min fitness initially " << fitness << " for index " << worstInd << " and # of clusters " << p->chromosome[worstInd]->active_ctr <<  endl;
  trackMinMaxAvg.close();
}
...
void DEMain::run(double min[], double max[], string filename) {
  ...
  trackArchive.open("Archives.txt");
  trackFile.open("clusters.txt");
  trackMinMaxAvg.open("popVaryingWine.txt", fstream::app);
  Population* archivePop = new Population(kmax,dim, generations);
  while (i < generations) {//till max generation reached
    Population* newpop = new Population(kmax, dim, 10*dim);
    double avgGen = 0.0;
    cout << "Before checking at the beginning of generation" << endl;
     cout << *(p->chromosome[p->bestChromosomeIndex]) << endl;
      ...
      //      cout << "After crossover " << p->bestChromosomeIndex << endl;
      if(p->bestChromosomeIndex == c){
	cout << "Offspring created for best meber of pop " <<endl;
	cout << *(offspring) << endl;
	cout << "Best member of pop " << endl;
	cout << *(p->chromosome[p->bestChromosomeIndex]) << endl;
	new_pop[c] = false;
	...
	archivePop->chromosome[i] = new Individual(*(p->chromosome[c]));
	// trackArchive << "Index " << c << " fitness " << p->chromosome[c]->rawFitness << " and cluster number " << p->chromosome[c]->active_ctr << endl;	  
      }
      else{		
	if (p->chromosome[c]->rawFitness <= offspring->rawFitness) {//if offspring better than parent, replace the parent
	  ...
	  new_pop[c] = false;
	  delete offspring;
	  newpop->chromosome[c] = new Individual(*(p->chromosome[c]));
	}
      }
    }//end for c
    ...
    // 			  }
    // 			}
    trackMinMaxAvg << "for generation " << i << " : " << endl;
    delete p;
    //			delete[] p->chromosome;
    ...
    cout << "After current population changed " << p->bestChromosomeIndex << endl;
    cout << "At the end " << endl;
    cout << *(p->chromosome[bestInd]) << endl;
    trackMinMaxAvg << "Max fitness " << fitness << " for index " << bestInd << " and # of clusters " << p->chromosome[bestInd]->active_ctr << endl;
    int worstInd = 0;
    fitness = p->chromosome[0]->rawFitness;
    for (int k = 1; k < pSize; k++) { //keeping track of the best member of the population
      if (fitness > p->chromosome[k]->rawFitness) {
    ...
    p->worstChromosomeIndex = worstInd;
    trackMinMaxAvg << "Min fitness " << fitness << " for index " << worstInd << " and # of clusters " << p->chromosome[worstInd]->active_ctr << endl;
    for (int k = 0; k < pSize; k++) {//for testing purposes (keeping track of the fitness change as the algorithm progresses)
      avgGen += p->chromosome[k]->rawFitness;
      trackFile << setiosflags(ios::left) ;
      trackFile << setw(5) << i << setw(12) << p->chromosome[k]->rawFitness << setw(5) <<  k << endl;
    }
    trackMinMaxAvg << "Average fitness " << avgGen/pSize << endl;
    i++;
  }
  cout << endl;
  ...
  for (int i = 0; i < generations; i++){
    trackArchive << "fitness " << archivePop->chromosome[i]->rawFitness << " and cluster # " << archivePop->chromosome[i]->active_ctr << endl;
  }
  trackFile.close();
  trackMinMaxAvg.close();
  trackArchive.close();
